...
def download(picure_path, json_path, output_path):
    ...
    save_setup_conf(
        picture_path=picure_path,
        json_file=json,
        dow_output=output_path,
    )
    ...
    with ui.tab_panels(tabs, value=one).classes("w-full"):
        with ui.tab_panel(one).style("background: #B1D3EF;"):
            with ui.card().classes(" mx-auto mt-20").style("width:75%"):
                ...
                i = (
                    ui.input(value=setup_conf["url"])
                    .props("clearable")
                    .classes(" w-full")
                    .props("outlined")
                )
                ui.label(
                    "Your API key — either a Personal Access Token (PAT) or legacy access token"
                )
                d = (
                    ui.input(value=setup_conf["api_key"], password=True)
                    .props("clearable")
                    .classes("w-full")
                    .props("outlined")
                )
                ui.label(
                    "The numeric ID of the project you want to download images from"
                )
                project_id_number = (
                    ui.number(value=setup_conf["project_id"])
                    .classes("w-full")
                    .props("outlined")
                )
                ui.button(
                    "Start",
                    on_click=lambda: try_connection(
                        i.value, d.value, int(project_id_number.value)
                    ),
                )
        with ui.tab_panel(two).style("background: #B1D3EF;"):
            with ui.card().classes(" mx-auto mt-20").style("width:75%"):
                ...
                local_path = (
                    ui.input(value=setup_conf["picture_path"])
                    .props("clearable")
                    .classes(" w-full")
                    .props("outlined")
                )
                ...
                json_path = (
                    ui.input(value=setup_conf["json_path"])
                    .props("clearable")
                    .classes(" w-full")
                    .props("outlined")
                )
                ...
                ui.button(
                    ...
                    on_click=lambda: download(
                        local_path.value, json_path.value, output.value
                    ),
                )
